...
//   - buildListings(): 카탈로그+더미로 매물 목록 생성
...
import { pickTerminalPair } from '../data/cable-terminals';
import { DUMMY_CATALOG, INCLUDE_DUMMY } from '../data/dummy-catalog';

// CATALOG에 실제 매물 데이터가 있는 브랜드
export const SOURCE_BRANDS = ['McIntosh', 'Marantz'];
...
export const SPEAKER_REDIST = ['북쉘프 스피커', '플로어 스탠딩 스피커', '톨보이 스피커', '센터 스피커', '사운드바', '서브우퍼'];
...
export function pickPrice(seed: number): number {
  const min = 50;
  const max = 1500;
