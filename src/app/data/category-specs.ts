...
export const AMP_SPEC_FIELDS: CategorySpecField[] = [
  ...
  { key: 'channel', label: '채널', input: { kind: 'select', options: AMP_CHANNEL_OPTS }, showWhen: (s) => !isAvr(s) && !isHpAmp(s) }, // AVR=채널 수 / 헤드폰 앰프=스테레오 전용이라 숨김
  ...
  { key: 'toneControl', label: '톤 컨트롤', input: { kind: 'select', options: YES_NO_OPTS }, showWhen: (s) => !isHpAmp(s) }, // 헤드폰 앰프엔 드묾
  { key: 'remote', label: '리모컨', input: { kind: 'select', options: YES_NO_OPTS }, showWhen: (s) => !isHpAmp(s) }, // 헤드폰 앰프엔 드묾
